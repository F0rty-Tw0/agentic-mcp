export const SKILL_CONTENT_INTRO_LINES = [
  ...
  '## Overview',
  '',
  'Use this skill when the goal is a working outcome: get a real answer from the provider already installed on the machine, compare multiple providers on one prompt, or configure one MCP client entry instead of wiring providers separately. Start by discovering providers, use `prove` for the fastest first real answer, then use `ask_<provider>` for focused work and `ask_all` only for deliberate comparison.',
  '',
  '## When to Use',
  '',
  ...
  '| Need the fastest first real proof | none | `agentic-mcp prove` or `agentic-mcp prove claude` | Default path for the first successful answer |',
  '| Need limited proof for one provider | `ping_<provider>` | `agentic-mcp ping_<provider>` | Before a manual provider-specific ask, not as final proof |',
  ...
  '| Need one provider to answer a task | `ask_<provider>` | `agentic-mcp ask_<provider> "..."` | Focused single-provider work after the first proof |',
  ...
  '3. **Check limited proof only if needed**: `ping_<provider>` for the provider you intend to inspect more closely.',
  '4. **Understand**: `help_<provider>` only when you need supported models, flags, or behavior details.',
  '5. **Prove real usage**: `prove` for the fastest first answer, `ask_<provider>` for focused work, `ask_all` only for side-by-side comparison.',
  ...
  '',
  'Then verify in order: `list_providers` -> `prove` -> `ask_<provider>` for focused work.',
] as const;
