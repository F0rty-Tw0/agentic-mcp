...
const runProviderAsk = async (
  provider: ResolvedProviderEntry,
  args: AskAllToolArgs
): Promise<AskAllProviderResult> => {
...
export const handleAskAll = async (
  ...
  if (filtered.length === 0) {
    return {
      isError: true,
      content: [{ type: 'text', text: 'No matching providers found. Check the providers filter or configure providers.' }],
    };
  }
