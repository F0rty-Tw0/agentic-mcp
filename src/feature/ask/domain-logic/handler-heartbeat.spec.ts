...
describe('handleAsk — progress heartbeat', () => {
  ...
  it('GIVEN extra with progressToken WHEN 30s elapses THEN sends progress notification', async () => {
  ...
  it('GIVEN extra with progressToken WHEN 90s elapses THEN sends 3 notifications with incrementing progress', async () => {
    ...
    expect(notifications[0]).toMatchObject({ params: { progress: 1 } });
    expect(notifications[1]).toMatchObject({ params: { progress: 2 } });
    expect(notifications[2]).toMatchObject({ params: { progress: 3 } });
  ...
  it('GIVEN extra without progressToken WHEN handling ask THEN no notifications are sent', async () => {
    const context = createContext();
